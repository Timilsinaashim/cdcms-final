...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-3 text-center">
            <?php
            // Validate and construct image source
            $imageSource = validate_image(isset($id) ? "uploads/babysitters/{$id}.png" : "");
            if (!empty($date_updated) && strtotime($date_updated) !== false) {
                $imageSource .= "?v=" . strtotime($date_updated);
            }
            ?>
            <!-- Display the image -->
            <img id="bs-img-modal" src="<?= htmlspecialchars($imageSource) ?>" alt="BS Image" class="bg-gradient-dark">
        </div>
    </div>
</div>
